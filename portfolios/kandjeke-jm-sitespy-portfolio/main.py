...
def main(page: ft.Page):
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Row([chip("Agriculture"), chip("Frontend", GOLD)], wrap=True, spacing=10),
            ft.Text("Johannes M Kandjeke", size=34, weight=ft.FontWeight.W_900, color=TEXT),
            ft.Text("GitHub: kandjekejohannes54 | Email: Kandjekejohannes54@gmail.com", color=MUTED, selectable=True),
            ft.Text("Responsive testing documentation and typography/spacing checks.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/kandjekejohannes54", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ft.Container(tile('Profile', [
                    ft.Text("Department: Agriculture", color=TEXT),
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Responsive testing documentation and typography/spacing checks.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- docs/User_Manual.md', color=MUTED, size=13),
                    ft.Text('- docs/Testing_Checklist.md', color=MUTED, size=13),
                    ft.Text('- src/theme/spacing.js', color=MUTED, size=13),
                    ft.Text('- src/theme/typography.js', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Johannes M Kandjeke\"\ngit config user.email \"Kandjekejohannes54@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/kandjeke-jm-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Johannes M Kandjeke SiteSpy portfolio\"\ngit push -u origin portfolio/kandjeke-jm-sitespy-portfolio", color=TEXT, selectable=True),
