...
def main(page: ft.Page):
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("GitHub: beathapawana-dot | Email: beathahaipumbu1@gmail.com", color=MUTED, selectable=True),
            ft.Text("Estimation formulas and save workflow.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/beathapawana-dot", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Estimation formulas and save workflow.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- src/utils/calculations.js', color=MUTED, size=13),
                    ft.Text('- src/services/estimateService.js', color=MUTED, size=13),
                    ft.Text('- src/screens/EstimateSummaryScreen.js', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Haipumbu Beatha NP\"\ngit config user.email \"beathahaipumbu1@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/haipumbu-bnp-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Haipumbu Beatha NP SiteSpy portfolio\"\ngit push -u origin portfolio/haipumbu-bnp-sitespy-portfolio", color=TEXT, selectable=True),
