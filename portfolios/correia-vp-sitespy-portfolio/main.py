...
def main(page: ft.Page):
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("GitHub: Valentina-Correia | Email: penneckyvalentina@gmail.com", color=MUTED, selectable=True),
            ft.Text("BOQ report summary and cost cards.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/Valentina-Correia", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("BOQ report summary and cost cards.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- src/services/reportService.js', color=MUTED, size=13),
                    ft.Text('- src/components/EstimateResultCard.js', color=MUTED, size=13),
                    ft.Text('- src/components/CostBreakdownCard.js', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Valentina Correia\"\ngit config user.email \"penneckyvalentina@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/correia-vp-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Valentina Correia SiteSpy portfolio\"\ngit push -u origin portfolio/correia-vp-sitespy-portfolio", color=TEXT, selectable=True),
