...
def main(page: ft.Page):
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("GitHub: amaliamangundu-tech | Email: amaliamangundu@gmail.com", color=MUTED, selectable=True),
            ft.Text("Profile, settings, and reusable buttons.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/amaliamangundu-tech", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Profile, settings, and reusable buttons.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- src/screens/ProfileScreen.js', color=MUTED, size=13),
                    ft.Text('- src/screens/SettingsScreen.js', color=MUTED, size=13),
                    ft.Text('- src/components/AppButton.js', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Amalia Mangundu\"\ngit config user.email \"amaliamangundu@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/mangundu-a-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Amalia Mangundu SiteSpy portfolio\"\ngit push -u origin portfolio/mangundu-a-sitespy-portfolio", color=TEXT, selectable=True),
