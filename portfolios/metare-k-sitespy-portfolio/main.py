...
def main(page: ft.Page):
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("GitHub: Kavakuru-Metarere7 | Email: kavakurum@gmail.com", color=MUTED, selectable=True),
            ft.Text("Authentication screens and auth service integration.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/Kavakuru-Metarere7", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Authentication screens and auth service integration.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- src/screens/LoginScreen.js', color=MUTED, size=13),
                    ft.Text('- src/screens/RegisterScreen.js', color=MUTED, size=13),
                    ft.Text('- src/screens/ForgotPasswordScreen.js', color=MUTED, size=13),
                    ft.Text('- src/services/authService.js', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Kavakuru Metarere\"\ngit config user.email \"kavakurum@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/metare-k-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Kavakuru Metarere SiteSpy portfolio\"\ngit push -u origin portfolio/metare-k-sitespy-portfolio", color=TEXT, selectable=True),
